        ...
        {data.home[0].frontPage.map((data, key)=>(
            <div key={key} className={`mt-2 columns-${Object.keys(data).length}`}>
                {Object.entries(data).map((dataCard, keyCard)=>(
                    <Link href={`${dataCard[1][0].link}`} key={keyCard}>
                        <div className={`shadow-md h-auto backdrop-blur-sm overflow-hidden border-2 p-4 rounded-lg flex content-center hover:shadow-lg items-center flex-${dataCard[1][0].direction}`}>
                            <Image  
